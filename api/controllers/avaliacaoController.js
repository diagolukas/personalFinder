...
export const avaliacaoPersonal = async (req, res) => {
...
export const avaliacaoCreate = async (req, res) => {
  const { personal_id, cliente_id, comentario, estrelas } = req.body

  // se não informou estes atributos
  if (!personal_id || !cliente_id || !comentario || !estrelas) {
  ...
  try {
    ...
    const avaliacao = await Avaliacao.create({
      personal_id, cliente_id, comentario, estrelas, data: new Date()
    }, { transaction: t });

    await Personal.increment('soma',
      { by: estrelas, where: { id: personal_id }, transaction: t }
    );

    await Personal.increment('num',
      { by: 1, where: { id: personal_id }, transaction: t }
    );
...
export const avaliacaoDestroy = async (req, res) => {
  ...
  try {
    ...
    await Personal.decrement('soma',
      {
        by: avaliacao.estrelas,
        where: { id: avaliacao.personal_id },
        transaction: t
      }
    );

    await Personal.decrement('num',
      {
        by: 1,
...
export const dadosGerais = async (req, res) => {
  ...
  try {
    ...
    const personals = await Personal.count()
    const media = await Personal.findOne({
    ...
    //    res.status(200).json({ clientes, Personals, media: media ? media.preco : 0, avaliacoes, avaliacoes_dia })
    res.status(200).json({ clientes, personals, media, avaliacoes, avaliacoes_dia })
  } catch (error) {
    res.status(400).send(error)
  }
}